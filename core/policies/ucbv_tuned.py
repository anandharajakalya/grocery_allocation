...
import numpy as np
from core.policies.ucbv import UCBV
...
class UCBVtuned(UCBV):
    ...
    def compute_index(self, arm):
        ...
        Where :math:`V'_k(t)` is an other estimator of the variance of rewards, obtained from :math:`X_k(t) = \sum_{
        \sigma=1}^{t} 1(A(\sigma) = k) r_k(\sigma)` is the sum of rewards from arm k, and :math:`Z_k(t) = \sum_{
        \sigma=1}^{t} 1(A(\sigma) = k) r_k(\sigma)^2` is the sum of rewards *squared*.
        """
        if self.pulls[arm] < 1:
            return float('+inf')
        else:
